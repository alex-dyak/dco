...
<?php if ( comments_open() ) : ?>
	...
	<div id="respond">
		...
		<h2><?php comment_form_title( __( 'Leave a Reply', 'w4ptheme' ), __( 'Leave a Reply to %s', 'w4ptheme' ) ); ?></h2>
		...
		<?php if ( get_option( 'comment_registration' ) && ! is_user_logged_in() ) : ?>
			<p><?php esc_html_e( 'You must be', 'w4ptheme' ); ?>&nbsp;
				<a href="<?php echo esc_url( wp_login_url( get_permalink() ) ); ?>"><?php esc_html_e( 'logged in', 'w4ptheme' ); ?></a>&nbsp;
				<?php esc_html_e( 'to post a comment.', 'w4ptheme' ); ?>
			</p>
		<?php else : ?>
			...
			<form
				...
				<?php if ( is_user_logged_in() ) : ?>
					<p>
						<?php esc_html_e( 'Logged in as', 'w4ptheme' ); ?>
						...
						<a href="<?php echo esc_attr( wp_logout_url( get_permalink() ) ); ?>"
						   title="Log out of this account"><?php esc_html_e( 'Log out', 'w4ptheme' ); ?> &raquo;</a>
					</p>
				...
				<?php else : ?>
					...
					<div>
						...
						<label
							for="author"><?php esc_html_e( 'Name', 'w4ptheme' ); ?> <?php echo $req ?: '(required)'; ?></label>
					</div>
					...
					<div>
						...
						<label
							for="email"><?php esc_html_e( 'Mail (will not be published)', 'w4ptheme' ); ?> <?php echo $req ?: '(required)'; ?></label>
					</div>
					...
					<div>
						<input type="text" name="url" id="url"
							   ...
							   size="22" tabindex="3"/>
						<label
							for="url"><?php esc_html_e( 'Website', 'w4ptheme' ); ?></label>
					</div>
				...
				<div>
					<input name="submit" type="submit" id="submit" tabindex="5"
						   value="<?php esc_attr_e( 'Submit Comment', 'w4ptheme' ); ?>"/>
